...
`create_sandboxed_codex_backend(...)` automatically creates the two Docker
networks and one shared egress container when sandbox mode is first used. An
...
The compose project creates the same resources as the automatic path:

- internal network `soveren-sandbox-egress`
...
public OpenAI connectivity, and confirms private/metadata destinations are
denied.
